...
| [`bundle-builder`](./bundle-builder) | A structured elicitation wizard for agent design — requirements engineering applied to "I want an agent that does X." It walks a five-question architecture (Identity, Routing, Capability, Memory, Boundary) plus a scaling axis (volume / variation / sophistication), refusing vague answers instead of accepting them, then scaffolds the resulting folder structure from a template system (`TEMPLATES/`). Includes three worked examples showing the output at different scales, from a single-job agent to a multi-location, multi-variant one. |
